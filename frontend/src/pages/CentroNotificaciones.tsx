...
import { listFichadas, fichadaAlmuerzo, createFichada, resetearDia, type Fichada } from '../lib/fichadasApi';
...
export function CentroNotificaciones() {
  ...
  // Demo/didáctico: resetea el día a cero para TODOS los empleados (vacía
  // fichadas + borra novedades automáticas). Un click, sin confirmación.
  async function resetearDiaDemo() {
    setVaciando(true);
    setError(null);
    try {
      await resetearDia(todayIso());
      await load();
    } catch (err) {
      setError(err instanceof ApiError ? err.message : 'Error al resetear el día');
    } finally {
      setVaciando(false);
    }
  }
